...
/* ============================================================
   РЯДОВЫЕ ЧТЕНИЯ (АПОСТОЛ И ЕВАНГЕЛИЕ)
   Подгрузка JSON-базы, расчет седмицы и вывод текстов на сегодня
   ============================================================ */
...
async function displayTodayReading() {
    // 1. Загружаем данные из папки reading-data
    const paschaDates = await loadJson('reading-data/pascha-dates.json');
    const ordinaryCycle = await loadJson('reading-data/ordinary-cycle.json');

    if (!paschaDates || !ordinaryCycle) return;

    // 2. Берем сегодняшнюю дату
    const today = new Date();

    // 3. Вычисляем позицию с помощью модуля LiturgicalCycle (из cycle.js)
    const pascha = LiturgicalCycle.pickPascha(today, paschaDates);
    const position = LiturgicalCycle.resolveCyclePosition(today, pascha);

    // 4. Достаем чтения из базы
    if (position.period !== 'unresolved') {
        // Идем по дереву: pentecost_period -> 16 -> tue
        const dayReading = ordinaryCycle[position.period]?.[position.week]?.[position.weekday];
        
        // Достаем название недели (например, "Неделя 1-я по Пятидесятнице")
        const weekNote = ordinaryCycle[position.period]?.[position.week]?.sun?.note || '';

        if (dayReading) {
            console.log(`Сегодня: ${weekNote}`);
            console.log(`Апостол: ${dayReading.apostle} (зач. ${dayReading.apostle_zachalo})`);
            console.log(`Евангелие: ${dayReading.gospel} (зач. ${dayReading.gospel_zachalo})`);

            // TODO: Привязка к HTML. Раскомментируй этот блок и замени ID на те, что добавишь в index.html
            /*
            const apostleContainer = document.getElementById('apostle-reading');
            const gospelContainer = document.getElementById('gospel-reading');
            if (apostleContainer) apostleContainer.textContent = `Апостол: ${dayReading.apostle}`;
            if (gospelContainer) gospelContainer.textContent = `Евангелие: ${dayReading.gospel}`;
            */
        } else {
            console.log("Рядовых чтений на этот день не найдено (возможно, великий праздник).");
        }
    }
}

// Запускаем расчет чтений после полной загрузки HTML
document.addEventListener("DOMContentLoaded", displayTodayReading);
...
// защита от XSS — экранируем спецсимволы перед вставкой через innerHTML,
...
// ключевые слова для выделения престольных праздников Никольского храма
...
function renderSchedule(services, phone) {
    ...
    // показываем 8 ближайших вместо 4
    ...
    if (servicesToShow.length === 0) {
        ...
        servicesToShow.forEach(service => {
            ...
            // подсветка престольных праздников
            ...
            // экранирование через escapeHtml() — защита от XSS (данные приходят из внешней Google-таблицы)
            li.innerHTML = `
